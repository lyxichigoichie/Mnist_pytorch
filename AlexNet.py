...
class AlexNet(nn.Module):
    ...
    def forward(self, x, ac_func):
        # out_conv1 = F.relu(self.conv1(x))
        out_conv1 = ac_func(self.conv1(x))
        out_pool1 = self.maxpool1(out_conv1)
        out_conv2 = ac_func(self.conv2(out_pool1))
        # out_conv2 = F.relu(self.conv2(out_pool1))
        out_pool2 = self.maxpool2(out_conv2)
        out_conv3 = ac_func(self.conv3(out_pool2))
        out_conv4 = ac_func(self.conv4(out_conv3))
        out_conv5 = ac_func(self.conv5(out_conv4))
        out_pool3 = self.maxpool3(out_conv5)
        
        flatten_x = self.flatten(out_pool3)
        out_linear1 = ac_func(self.linear1(flatten_x))
        out_dropout1 = self.dropout1(out_linear1)
        out_linear2 = ac_func(self.linear2(out_dropout1))
        out_dropout2 = ac_func(out_linear2)
        out_linear3 = ac_func(self.linear3(out_dropout2))
        return out_linear3